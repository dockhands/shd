/**
 * Created by Dan Hawkins, but adapted (ie basically stolen) from Adrian Cooney; http://dunxrion.github.io
...
// [img]http://i.imgur.com/hnnkCJp.jpg?1[/img]
	...
	function drawMemeText(ctx, type, text, width, y) {
		...
		if(text.length < 24) {
			var val = Math.max(0, text.length - 60),
				size = 70 + (val * - 3);

			drawText(ctx, size, text, width/2-50, y);
		} else if(text.length < 40) {
			drawText(ctx, 40, text, width/2-50, y);
		} else {
